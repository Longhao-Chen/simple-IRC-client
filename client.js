...
function verifyLink(link) {
	...
	if (linkHref !== link.innerHTML) {
		return confirm('警告，请确认这是您要去的地方：' + linkHref);
	}
...
var frontpage = [
	"simple-IRC-client",
	"用于临时的通信，主要代码来自：https://github.com/hack-chat/main",
	"使用方法：https://longhao-chen.github.io/simple-IRC-client?频道号",
	"这里有一个为您生成的随机值： ?" + Math.random().toString(36).substr(2, 8),
].join("\n");
				...
				if (result === "granted") {
					...
					if (notifyPermissionExplained === 0) {
						pushMessage({
							cmd: "chat",
							nick: "*",
							text: "通知权限被拒绝，如果有人@提到你，你将不会得到通知。",
							time: null
						});
						notifyPermissionExplained = -1;
					}
...
function join(channel) {
	//使用hack.chat的服务器
	if (true || document.domain == 'hack.chat') {
		// For https://hack.chat/
	...
	ws.onopen = function () {
		...
		if (!wasConnected) {
			if (location.hash) {
				...
				var newNick = prompt('昵称（不要包含中文和特殊字符）', myNick);
				if (newNick !== null) {
					myNick = newNick;
				} else {
	...
	ws.onclose = function () {
		if (wasConnected) {
			pushMessage({ nick: '!', text: "服务器已断开连接。正在尝试重新连接. . ." });
		}
...
function setHighlight(scheme) {
	...
	$('#highlight-link').href = "https://cdn.bootcdn.net/ajax/libs/highlight.js/11.2.0/styles/" + scheme + ".min.css";
